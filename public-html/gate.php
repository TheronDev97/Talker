<?php
    ...
    $_SESSION["resend"] = empty($_SESSION["resend"]) ? '' : $_SESSION["resend"]; //to prevent undefined index error

    if (empty($dbUserRow["user_verified"]) && empty($_SESSION["resend"])){
        $_SESSION["msgid"]="809";
    }
?>
...
<html lang="en">
	...
	<div class="container">
        ...
        <br>
        <?php 
        $_COOKIE["cookieUserEmail"] = isset($_COOKIE["cookieUserEmail"]) ? $_COOKIE["cookieUserEmail"] : ''; //prevent error
        echo "cookieUserEmail: " . $_COOKIE["cookieUserEmail"]; 
        ?>
        <br>
        <?php 
        $_COOKIE["cookieUserPassword"] = isset($_COOKIE["cookieUserPassword"]) ? $_COOKIE["cookieUserPassword"] : '';
        echo "cookieUserPassword: " . $_COOKIE["cookieUserPassword"]; 
        ?>
